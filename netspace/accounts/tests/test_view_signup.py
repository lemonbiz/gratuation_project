...
class SignUpTest(TestCase):
    ...
    def test_form_inputs(self):
        """
                The view must contain five inputs: csrf, username, em
        ail, password1, password2, 后面的数字是出现次数
        """
...
class SuccessfulSignUpTests(TransactionTestCase):
    def setUp(self):
        url = reverse('signup')
        data = {
            'username': 'john',
...
class InvalidSignUpTests(TransactionTestCase):
    def setUp(self):
        url = reverse('signup')
